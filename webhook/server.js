...
// Load Mongoose model for storing user tokens
...
// Note: We set up a custom "verify" function for bodyParser below for signature checks
...
function verifyGithubSignature(req, res, buf) {
  ...
  if (!signature) {
    throw new Error('[Webhook] Missing X-Hub-Signature-256 header.');
  }

  // Compare GitHub's signature with our computed HMAC
...
// If you want to verify signatures, override body-parser's "verify" option
app.use(
  bodyParser.json({
    verify: (req, res, buf) => {
